...
// Convert graph state to legacy workflow state
export async function getWorkflowState(projectPath?: string | null): Promise<WorkflowState> {
...
export async function saveWorkflowState(_state: WorkflowState, projectPath?: string | null): Promise<boolean> {
...
export async function getWorkflowSteps(projectPath?: string | null): Promise<WorkflowStep[]> {
  const graph = await getGraph(projectPath);
  if (!graph) {
...
export function getDefaultSteps(): WorkflowStep[] {
  return [
    {
      id: 'complexity-check',
      order: 0,
...
export async function getGlobalWorkflowSteps(workflowName: string): Promise<WorkflowStep[]> {
  const graph = await getGlobalGraph(workflowName);
  if (!graph) return [];
...
export async function getWorkflowSettings(_projectPath?: string | null): Promise<WorkflowSettings> {
  return {
...
export async function saveWorkflowSettings(_settings: WorkflowSettings, _projectPath?: string | null): Promise<boolean> {
  // Settings are now embedded in the graph YAML
  // For now, return true as a no-op
  return true;
}

export async function saveWorkflowSteps(_steps: WorkflowStep[], _projectPath?: string | null): Promise<boolean> {
  // Converting steps to graph format would require significant work
  // For now, this is a no-op - use graph YAML directly
  console.warn('[Graph] saveWorkflowSteps is deprecated. Edit graph.yaml directly.');
  return true;
}
