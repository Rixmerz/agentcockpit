...
export function McpPanel({
  projectPath,
  onMcpsChange: _onMcpsChange, // New API (for future use)
  ...
  // Note: _onMcpsChange is the new unified API, keeping legacy props for backwards compatibility
  ...
  const handleRemoveCode = useCallback(async (name: string) => {
    try {
      ...
      if (config === null) {
        showMessage('error', 'Could not read Claude configuration');
        return;
      }

      if (!config?.mcpServers?.[name]) {
        showMessage('error', `"${name}" not found in Code`);
        return;
      }
  ...
  const handleOpenConfigInIDE = useCallback(async (configType: 'desktop' | 'code') => {
    try {
      ...
      showMessage('error', `Error opening config: ${e}`);
    }
                  ...
                  return (
                    <div key={`desktop-${server.name}`} className="mcp-item group">
                      <div
                        ...
                        title="Click to inject in session"
                      >
                        <Server size={14} style={{ color: isMarkedForRemoval ? 'var(--warning)' : isSelected ? 'var(--success)' : 'var(--text-muted)' }} />
                        ...
                        {isMarkedForRemoval && <Minus size={12} style={{ color: 'var(--warning)' }} />}
                      </div>

                      <div className="mcp-actions">
                        {/* Remove from session button (yellow) */}
                        <button
                          className={`btn-icon-sm ${isMarkedForRemoval ? 'warning-active' : 'warning'}`}
                          onClick={(e) => {
                            e.stopPropagation();
                            handleDoubleClick(server.name);
                          }}
                          title={isMarkedForRemoval ? 'Cancel remove from session' : 'Remove from active session'}
                        >
                          <Minus size={12} />
                        </button>
                        {!isInCode && (
