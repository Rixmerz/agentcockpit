...
import type { PipelineGraph, GraphState, GraphNode } from '../../services/pipelineService';
import { getGraph, getGraphState } from '../../services/pipelineService';
...
function computeAnalytics(graph: PipelineGraph, state: GraphState): PipelineAnalytics | null {
...
export function PipelineAnalyticsView({ projectPath }: PipelineAnalyticsViewProps) {
  ...
  if (!analytics) {
    return <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-muted)' }}>No pipeline data available yet.</div>;
  }
