...
import type { GraphVisualization } from '../../services/workflowService';
...
export function WorkflowModal({ isOpen, onClose, projectPath }: WorkflowModalProps) {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [workflowState, workflowSteps, workflowSettings, mcps, gState, edges, viz] = await Promise.all([
        getWorkflowState(projectPath),
        getWorkflowSteps(projectPath),
        getWorkflowSettings(projectPath),
        getAvailableMcps(),
      ...
      console.error('[WorkflowModal] Failed to load:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveSettings = async () => {
    ...
    setSaving(true);
    try {
      await saveWorkflowSettings(settings, projectPath);
      await loadData();
      setViewMode('status');
    } finally {
      setSaving(false);
  ...
  const renderStatusView = () => (
    <>
      ...
      <div className="workflow-steps-status">
        {steps.map((step, index) => {
          const status = getStepStatus(index);
          const visits = graphState?.node_visits[step.id] || 0;
          return (
            <div key={step.id} className={`workflow-step-status workflow-step-${status}`}>
              ...
              <div className="workflow-step-info">
                ...
                {visits > 0 && (
                  <span className="workflow-step-visits">
                    <Repeat size={10} />
      ...
      {availableEdges.length > 0 && (
        <div className="workflow-edges-modal">
          <h4><GitBranch size={14} /> Available Transitions</h4>
          <div className="workflow-edges-grid">
            {availableEdges.map((edge) => (
              <div key={edge.id} className="workflow-edge-card">
  ...
  const renderGraphView = () => (
    <>
      ...
      {graphViz?.currentNode && (
        <div className="workflow-current-node-info">
      ...
      {availableEdges.length > 0 && (
        <div className="workflow-edges-section">
          ...
          <div className="workflow-edges-list">
            {availableEdges.map((edge) => (
              <div key={edge.id} className="workflow-edge-item">
                <ArrowRight size={14} />
                <span className="edge-target">{edge.toName}</span>
      ...
      <div className="workflow-steps-list">
        {steps.map((step, index) => (
          <div key={step.id} className="workflow-step-item">
            <div className="workflow-step-order">{index}</div>
            <div className="workflow-step-content">
              <div className="workflow-step-header">
                <span className="workflow-step-name">{step.name}</span>
                <span className={`workflow-step-gate gate-${step.gate_type}`}>
                  {step.gate_type}
                </span>
              </div>
        ...
        <div className="settings-section">
          ...
          <div className="workflow-checkbox-grid">
            {availableMcps.map(mcp => (
              <label key={mcp.name} className="workflow-checkbox-item">
                <input
                  type="checkbox"
        ...
        <div className="settings-section">
          ...
          <div className="workflow-checkbox-grid">
            {STANDARD_TOOLS.map(tool => (
              <label key={tool} className="workflow-checkbox-item">
                <input
                  type="checkbox"
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Workflow Control">
